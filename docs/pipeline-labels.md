...
| `agent:run` | `#0E8A16` | Triggers the Claude workflow to process the issue | When scope is clear and Claude should execute the task | Human (SHIHCHENG) adds to kick off; workflow removes after pickup |
...
- `agent:run` → `agent:stuck`: workflow removes `agent:run`, adds `agent:stuck` after repeated failure + refusal log entry.
- `agent:run` → `agent:needs-user-decision`: workflow removes `agent:run`, adds `agent:needs-user-decision` when Claude halts pending human input.
